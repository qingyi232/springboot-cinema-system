<template>
  <div>
    <el-space direction="vertical" alignment="left" style="width: 100%">
      ...
      <el-card>
        <el-alert title="收藏记录页已统一为后台记录视图：可以清晰查看用户收藏了哪部电影，以及收藏时间。" type="info" :closable="false" style="margin-bottom: 16px" />
        <el-table ref="tableComponents" :data="listData" tooltip-effect="dark" style="width: 100%" @selection-change="selectionChange" border>
          ...
          <el-table-column label="电影信息" min-width="220">
            <template #default="scope">
              ...
              <div class="record-sub">收藏来源：用户主动收藏</div>
            </template>
          </el-table-column>
          <el-table-column label="用户信息" width="180">
            <template #default="scope">
              <el-tag type="warning" effect="plain" round>{{ scope.row.username || '--' }}</el-tag>
            </template>
          </el-table-column>
...
function getPageList() {
  ...
  request.get("/movieCollect/page", { params: data }).then(res => {
...
function batchDelete(rows) {
  ...
  ElMessageBox.confirm(`此操作将永久删除ID为[${ids}]的数据, 是否继续?`, '提示', { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning', center: true }).then(() => {
    request.delete("/movieCollect/delBatch", { data: ids }).then(res => {
      if (!res) return
